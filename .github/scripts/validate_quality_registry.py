...
QUALITY = ROOT / "registry" / "quality"
PROFILE_SCHEMA = QUALITY / "quality-profile.schema.json"
...
for schema in (PROFILE_SCHEMA, ANALYSIS_SCHEMA):
...
profile_paths = sorted(
    path
    for path in QUALITY.rglob("*.json")
...
for path in profile_paths:
    ...
    if profile_id:
        ...
        if profile_id in profiles:
            fail(
                f"duplicate quality profile id {profile_id}: "
                f"{profiles[profile_id].relative_to(ROOT)} and {path.relative_to(ROOT)}"
            )
    ...
    if not isinstance(capabilities, list):
        fail(f"{path.relative_to(ROOT)}: capabilities must be a list")
    else:
        if len(capabilities) != len(set(capabilities)):
            fail(f"{path.relative_to(ROOT)}: capabilities must be unique")
        for capability in capabilities:
            if capability not in CAPABILITIES:
                fail(
                    f"{path.relative_to(ROOT)}: invalid capability {capability!r}"
                )
    ...
    if not isinstance(sources, list) or not sources:
        ...
        for source in sources:
            ...
            if source.get("authority") not in AUTHORITIES:
                fail(
                    f"{path.relative_to(ROOT)}: source {source_id!r} has invalid authority"
                )
            url = source.get("url")
            if not isinstance(url, str) or not url.startswith("https://"):
                fail(
                    f"{path.relative_to(ROOT)}: source {source_id!r} must use an https URL"
                )
            date = source.get("reviewed_at")
            if not isinstance(date, str) or not DATE.match(date):
                fail(
                    f"{path.relative_to(ROOT)}: source {source_id!r} reviewed_at must be YYYY-MM-DD"
                )
    ...
    for rule in rules:
        ...
        if not isinstance(rule_id, str) or not ID.match(rule_id):
            ...
            if rule_id in all_rule_ids:
                fail(f"duplicate global quality rule id: {rule_id}")
            all_rule_ids.add(rule_id)

        if rule.get("category") not in CATEGORIES:
            fail(f"{path.relative_to(ROOT)}: rule {rule_id!r} has invalid category")
        ...
        if rule.get("enforceability") not in ENFORCEABILITY:
            fail(
                f"{path.relative_to(ROOT)}: rule {rule_id!r} has invalid enforceability"
            )
        if rule.get("authority") not in AUTHORITIES:
            fail(f"{path.relative_to(ROOT)}: rule {rule_id!r} has invalid authority")
        if rule.get("autofix") not in AUTOFIX:
            fail(f"{path.relative_to(ROOT)}: rule {rule_id!r} has invalid autofix")
        if not isinstance(rule.get("constraints"), dict):
            fail(
                f"{path.relative_to(ROOT)}: rule {rule_id!r} constraints must be an object"
            )
        ...
        if not isinstance(tools, list) or any(
            ...
            fail(
                f"{path.relative_to(ROOT)}: rule {rule_id!r} tools must be a string list"
            )
        refs = rule.get("source_refs")
        if not isinstance(refs, list):
            fail(
                f"{path.relative_to(ROOT)}: rule {rule_id!r} source_refs must be a list"
            )
        else:
            for ref in refs:
                if ref not in source_ids:
                    fail(
                        f"{path.relative_to(ROOT)}: rule {rule_id!r} references unknown source {ref!r}"
                    )
...
for profile_id, path in profiles.items():
    ...
    for ref in composes:
        if ref not in profiles:
            fail(
                f"{path.relative_to(ROOT)}: composes_with references unknown profile {ref!r}"
            )
        if ref == profile_id:
            fail(f"{path.relative_to(ROOT)}: profile cannot compose with itself")

if errors:
    print("Quality registry validation failed:", file=sys.stderr)
    for error in errors:
        print(f"- {error}", file=sys.stderr)
    raise SystemExit(1)

print(
    f"Quality registry valid: {len(profiles)} profiles, "
    f"{len(all_rule_ids)} unique rules"
)
