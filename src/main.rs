...
fn template_meta(name: &str) -> Value {
    ...
    serde_json::from_str(&embedded_text(&TEMPLATES, &path).unwrap_or_else(|| die(format!("unknown template: {name}"))))
        .unwrap_or_else(|e| die(format!("invalid {path}: {e}")))
}
...
fn resolve(mut o: ComposeOpts) -> (Vec<String>, ComposeOpts) {
    if let Some(profile) = &o.profile {
        ...
        let data: Value = serde_json::from_str(&embedded_text(&PROFILES, &path).unwrap_or_else(|| die(format!("unknown profile: {profile}"))))
            .unwrap_or_else(|e| die(format!("invalid profile {profile}: {e}")));
        if o.maturity.is_none() { o.maturity = data.get("maturity").and_then(Value::as_str).map(str::to_string); }
    ...
    if o.packs.is_empty() { o.packs = strings(leaf.get("default_packs")); }
    if o.skills.is_empty() { o.skills = strings(leaf.get("default_skills")); }
    dedupe(&mut o.packs); dedupe(&mut o.skills); dedupe(&mut o.policies);
    (chain, o)
}
...
fn patch_manifest(path: &Path, name: Option<&str>, maturity: Option<&str>, packs: &[String]) -> io::Result<()> {
    ...
    if let Some(maturity) = maturity {
        for old in ["prototype", "startup", "production", "critical"] {
            ...
            if text.contains(&needle) { text = text.replacen(&needle, &format!("maturity: {maturity}"), 1); break; }
        }
    }
    if !packs.is_empty() {
        ...
        for line in text.lines() {
            ...
            if inside && !line.starts_with(' ') { inside = false; }
...
fn compose(target: &Path, o: ComposeOpts, preserve: bool) -> io::Result<Value> {
    ...
    for name in chain.iter().filter(|n| n.as_str() != "base") {
        if let Some(overlay) = embedded_dir(&TEMPLATES, &format!("{name}/overlay")) { created.extend(copy_embedded(overlay, target, preserve)?); }
    }
    ...
    Ok(json!({"templates":chain,"preset":o.preset,"profile":o.profile,"created":created,"packs":o.packs,"skills":o.skills,"policies":o.policies,"maturity":o.maturity}))
}
...
fn is_test_path(s: &str) -> bool {
    ...
    ["test","tests","spec","specs"].iter().any(|n| x.split(|c:char| "/_.-".contains(c)).any(|p| p == *n))
}
fn clamp(v: i64) -> i64 { v.clamp(0, 100) }

fn codebase_audit(root: &Path) -> Value {
    let files = all_files(root);
    let mut code = 0; let mut loc = 0i64; let mut large = Vec::new(); let mut todos = 0i64;
    let mut tests = 0; let mut workflows = 0; let mut docs = 0; let mut manifests = Vec::new(); let mut locks = Vec::new();
    let mut security = false; let mut agent = false; let mut ops = false;
    for path in &files {
        ...
        if CODE_EXT.contains(&ext.as_ref()) {
            ...
            if let Ok(text) = fs::read_to_string(path) {
                let n = text.lines().count() as i64; loc += n; if n > 800 { large.push(rel.clone()); }
                for marker in ["TODO","FIXME","HACK","XXX"] { todos += text.matches(marker).count() as i64; }
            }
        }
    }
    ...
    let mut overall = weights.iter().map(|(k,w)| scores[*k] * w).sum::<i64>() / weights.iter().map(|(_,w)| *w).sum::<i64>();
    ...
    json!({
        "overall":overall,"target_maturity":"unknown","scores":scores,
        "readiness":{"prototype":clamp(overall+15),"startup":clamp(overall+if has_tests&&ci{3}else{-8}),"production":clamp(overall-if has_tests&&ci&&security&&ops{8}else{20}),"critical":clamp(overall-if has_tests&&ci&&security&&ops{22}else{35})},
        ...
        "design_system":ds,"findings":findings,
        ...
        if let Ok(text) = fs::read_to_string(&path) {
            for (typ, needle) in [("private_key", marker.as_str()), ("private_key", rsa.as_str()), ("private_key", openssh.as_str()), ("aws_access_key", "AKIA")] {
                for (i, line) in text.lines().enumerate() {
                    if line.contains(needle) { findings.push(json!({"severity":"high","type":typ,"path":path.strip_prefix(root).unwrap_or(&path),"line":i+1})); }
                }
            }
        }
    }
...
fn parse_compose(args: &[String]) -> (PathBuf, ComposeOpts, bool) {
    ...
    let mut allow = false; let mut i = 1;
    while i < args.len() {
        match args[i].as_str() {
            "--template" => { i += 1; o.template = args.get(i).cloned().unwrap_or_else(|| die("--template requires value")); }
            "--preset" => { i += 1; o.preset = args.get(i).cloned(); }
            "--profile" => { i += 1; o.profile = args.get(i).cloned(); }
            "--pack" => { i += 1; o.packs.push(args.get(i).cloned().unwrap_or_else(|| die("--pack requires value"))); }
            "--skill" => { i += 1; o.skills.push(args.get(i).cloned().unwrap_or_else(|| die("--skill requires value"))); }
            "--policy" => { i += 1; o.policies.push(args.get(i).cloned().unwrap_or_else(|| die("--policy requires value"))); }
            "--name" => { i += 1; o.name = args.get(i).cloned(); }
            "--maturity" => { i += 1; o.maturity = args.get(i).cloned(); }
...
fn main() {
    ...
    if argv.len() < 2 || ["-h","--help"].contains(&argv[1].as_str()) { usage(prog); return; }
    let code = match argv[1].as_str() {
        "init" => {
            let (target, o, allow) = parse_compose(&argv[2..]); fs::create_dir_all(&target).unwrap();
            if !allow && fs::read_dir(&target).map(|mut x| x.next().is_some()).unwrap_or(false) { die("target is not empty; use --allow-existing or upgrade"); }
            let mut r = compose(&target, o, false).unwrap(); r["mode"] = json!("INIT"); r["target"] = json!(target); enrich_design_system(&target, &mut r); pretty(r); 0
        }
        "upgrade" => {
            let (target, o, _) = parse_compose(&argv[2..]); if !target.exists() { die("target does not exist"); }
            let mut r = compose(&target, o, true).unwrap(); r["mode"] = json!("UPGRADE"); r["target"] = json!(target); r["preserved_existing"] = json!(true); enrich_design_system(&target, &mut r); pretty(r); 0
        }
        "audit" => {
            let path = PathBuf::from(argv.get(2).map(String::as_str).unwrap_or(".")); let r = codebase_audit(&path);
            let fail = r["findings"].as_array().unwrap().iter().any(|f| matches!(f["severity"].as_str(), Some("high"|"critical"))); pretty(r); if fail { 1 } else { 0 }
        }
        "design-system-components" => {
            ...
            if write { let out = design_system::write_plan(&path).unwrap(); r["written"] = json!(out); }
            pretty(r); 0
        }
...
mod tests {
    use super::*;
    #[test] fn embedded_base_exists() { assert!(TEMPLATES.get_file("base/AGENTS.md").is_some()); }
    #[test] fn chain_works() { assert_eq!(template_chain("web-app".into()).first().unwrap(), "base"); }
    #[test] fn profile_exists() { assert!(PROFILES.get_file("startup/profile.json").is_some()); }
    #[test] fn secret_fixture() {
        let path = env::temp_dir().join(format!("ah-test-{}", std::process::id())); let _ = fs::remove_dir_all(&path); fs::create_dir_all(&path).unwrap();
        fs::write(path.join("x"), ["-----BEGIN ", "PRIVATE KEY-----"].concat()).unwrap(); assert!(!secret_scan(&path)["passed"].as_bool().unwrap()); let _ = fs::remove_dir_all(path);
    }
}
