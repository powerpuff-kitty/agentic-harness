...
fn corpus(root: &Path) -> String {
    ...
    for path in files(root) {
        ...
        out.push_str(&rel.to_ascii_lowercase());
        out.push('\n');
...
pub fn component_plan(root: &Path) -> Value {
    ...
    if user_facing {
        needed.extend(["tokens", "typography", "icon", "layout", "surface", "button", "icon-button", "alert", "spinner", "skeleton", "empty-state", "error-state", "tooltip"]);
    }
    if has_any(&text, &["login", "sign in", "signup", "register", "password", "authentication"]) {
        needed.extend(["form-field", "input", "checkbox", "button", "alert"]);
    }
    if has_any(&text, &["dashboard", "admin", "workspace", "console"]) {
        needed.extend(["app-shell", "header", "sidebar", "tabs", "card", "table", "stat"]);
    }
...
pub fn audit(root: &Path) -> Value {
    ...
    if !plan["active"].as_bool().unwrap_or(false) {
        return json!({"active":false,"score":null,"status":"not-required","plan":plan,"violations":[],"missing_components":[]});
    }

    let mut raw_controls = Vec::new();
    let mut hardcoded_colors = Vec::new();
    for path in files(root) {
        let rel = path.strip_prefix(root).unwrap_or(&path).to_string_lossy().replace('\\', "/");
        let lower = rel.to_ascii_lowercase();
        if lower.contains("component") || lower.contains("design-system") || lower.contains("storybook") { continue; }
        let Ok(text) = fs::read_to_string(&path) else { continue };
        let raw = ["<button", "<input", "<select", "<textarea"].iter().filter(|n| text.to_ascii_lowercase().contains(**n)).count();
        if raw > 0 { raw_controls.push(json!({"path":rel,"count":raw})); }
        let colors = hardcoded_color_count(&text);
        if colors > 0 { hardcoded_colors.push(json!({"path":rel,"count":colors})); }
    }
    ...
    if !missing.is_empty() { violations.push(json!({"severity":"medium","type":"missing-required-component","message":"Components inferred from product flows are not represented in the discovered design-system component paths.","evidence":missing})); }
...
mod tests {
    ...
    fn detects_dashboard_components() {
        ...
        let plan = component_plan(&root);
        let items = plan["needed_components"].as_array().unwrap();
        assert!(items.iter().any(|x| x == "sidebar"));
        assert!(items.iter().any(|x| x == "search"));
