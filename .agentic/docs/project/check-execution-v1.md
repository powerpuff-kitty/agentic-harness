...
## Additive timing and supervision clarification — 2026-09-18

Optional `halt_reason` records the sticky ledger stop (or null); the known reasons are supervisor integrity, input revalidation, budget exhaustion, invalid sequence and missing results, serialized as the ledger variant names. Optional `timing` fields preserve compatibility with earlier v1 reports while making new measurements explicit. Run timing records `review_ms`, scope `invocation-through-final-revalidation`, and the 250 ms cleanup/recovery grace allowances. Spawned-outcome timing separates `execution_ms`, `cleanup_ms` and `recovery_ms`. Durations are monotonic observations rounded down to milliseconds; OS scheduling/read calls are not hard realtime bounds. Cleanup may extend past the invocation deadline, but never convert an expired invocation to success.

All review/input/tool-read loops must check the same deadline, including final revalidation. Apply a 512 MiB cumulative native-tool read cap within each review (including executor identity), retaining the 256 MiB individual-file cap. Aliases of the same canonical identity may share one hash within that review only; every revalidation starts with an empty cache. A completion observed after the supported deadline is not successful completion, even if the child exited earlier.

On Linux/macOS, observe terminal child status with `waitid(WNOWAIT)` before the last group signal, then collect it. Never signal a group after reaping or lost ownership (`ECHILD`). Use verified platform ABI definitions. SIGINT/SIGTERM request cooperative cancellation and cleanup; SIGKILL/crashes cannot run this path. If bounded direct-child recovery fails, report an integrity error and retain wait responsibility in a non-signaling recovery owner; supervisor termination can still end that recovery. No guarantee of cleanup after uncatchable termination or uninterruptible OS behavior is implied.

On macOS, `killpg` can return `EPERM` when every group member is a zombie. This is not generally a successful cleanup signal. The additive cleanup result `no-live-group-members` is allowed only after bounded native inspection verifies no live members while the direct child is still owned and unreaped; inspection errors, truncation or live members remain failures. The outcome ledger accepts this observed cleanup state alongside `signal-sent-or-group-absent`. Older consumers that reject the new enum value must upgrade their pinned contract before consuming it.

Primary platform source: [Apple XNU killpg1](https://github.com/apple-oss-distributions/xnu/blob/main/bsd/kern/kern_sig.c) filters zombie group members and returns `EPERM` if no eligible member was found. This explains the observed local behavior; permission failures are not silently reclassified.
